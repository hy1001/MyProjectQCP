...
getPartsFromCompleteTextIQM <- function( modelText ) {
...
modelTextStructure <- list()

modelTextStructure$name = substr( modelText, nameStart + nchar( "********** MODEL NAME" ), notesStart - 1 )
modelTextStructure$notes =       substr( modelText, notesStart + nchar( "********** MODEL NOTES" ), statesStart - 1 ) 
modelTextStructure$states =      substr( modelText, statesStart + nchar('********** MODEL STATES'), parametersStart-1 );
modelTextStructure$parameters =  substr(  modelText , parametersStart + nchar( '********** MODEL PARAMETERS' ), variablesStart-1 );
modelTextStructure$variables =   substr( modelText, variablesStart + nchar('********** MODEL VARIABLES'), reactionsStart - 1 );
modelTextStructure$reactions =   substr( modelText, reactionsStart + nchar ('********** MODEL REACTIONS'), functionsStart - 1 );
modelTextStructure$functions =  substr(modelText, functionsStart + nchar('********** MODEL FUNCTIONS'), eventsStart-1);
modelTextStructure$events =     substr(modelText, eventsStart + nchar('********** MODEL EVENTS'), functionsRStart - 1 );
